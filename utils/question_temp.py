...
class QuestionSet:
    ...
    def has_actor_question(word_objects):
        ...
        for w in word_objects:
            if w.pos == pos_movie:
                e = "?m :movieTitle '{movie}'." \
                    "?m :hasActor ?a." \
                    "?a :personName ?x".format(movie=w.token)
    ...
    def has_cooperation_question(word_objects):
        ...
        if person1 is not None and person2 is not None:
            e = "?p1 :personName '{person1}'." \
                "?p2 :personName '{person2}'." \
                "?p1 :hasActedIn ?m." \
                "?p2 :hasActedIn ?m." \
                "?m :movieTitle ?x".format(person1=person1, person2=person2)
    ...
    def has_compare_question(word_objects):
        ...
        if person is not None and number is not None:
            ...
            e = "?p :personName '{person}'." \
                "?p :hasActedIn ?m." \
                "?m :movieTitle ?x." \
                "?m :movieRating ?r." \
                "filter(?r {mark} {number})".format(person=person, number=number,
                                                     mark=keyword)
        ...
        for w in word_objects:
            if w.pos == pos_person:
                e = "?s :personName '{person}'." \
                    "?s :hasActedIn ?m." \
                    "?m :hasGenre ?g." \
                    "?g :genreName ?x".format(person=w.token)
        ...
        for w in word_objects:
            if w.pos == pos_person:
                e = "?s :personName '{person}'." \
                    ...
                    "?m :hasGenre ?g." \
                    "?g :genreName '{keyword}'." \
                    "?m :movieTitle ?x".format(person=w.token, keyword=keyword)
    ...
    def has_quantity_question(word_objects):
        ...
        for w in word_objects:
            if w.pos == pos_person:
                e = "?s :personName '{person}'." \
                    "?s :hasActedIn ?x.".format(person=w.token)
    ...
    def is_comedian_question(word_objects):
        ...
        for w in word_objects:
            if w.pos == pos_person:
                e = "?s :personName '{person}'." \
                    "?s rdf:type :Comedian.".format(person=w.token)
    ...
    def has_basic_person_info_question(word_objects):
        ...
        for w in word_objects:
            if w.pos == pos_person:
                e = "?s :personName '{person}'." \
                    "?s {keyword} ?x.".format(person=w.token, keyword=keyword)
    ...
    def has_basic_movie_info_question(word_objects):
        ...
        for w in word_objects:
            if w.pos == pos_movie:
                e = "?s :movieTitle '{movie}'." \
                    "?s {keyword} ?x.".format(movie=w.token, keyword=keyword)
